# `@pkg/workers` — Cosmetic Workers

Phase 4 Sprint 2 (Legacy).

## Files

| 파일          | 역할                                                                    |
| ------------- | ----------------------------------------------------------------------- |
| `cosmetic.ts` | 메인 스레드 API + Graceful Degradation. (Web Worker pipeline disabled.) |
...
import {
  calcFortuneScore,
  calcMultiplierFrames,
  calcNearMiss,
  calcParticles,
} from "@/packages/workers/cosmetic";
...
3. `navigator.deviceMemory < 2` 또는 `hardwareConcurrency < 2` → Main thread
   fallback
