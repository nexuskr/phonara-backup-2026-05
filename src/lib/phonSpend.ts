 ...
 * - spendForBooster: 5,000 PHON → 24h 부스터 (수수료 -30% / Crown ×1.5 / 레버리지 7x)
 * - spendForCrownBoost: 1,000 PHON → 24h Crown ×1.5만
 */
import { supabase } from "@/integrations/supabase/client";

export async function spendPhonForFeeDiscount(amount: number) {
  const { data, error } = await supabase.rpc("spend_phon_for_fee_discount", { _amount: amount });
...
export async function spendPhonForCrownBoost() {
