...
const CHANNELS: { id: ShareChannel; label: string; Icon: any }[] = [
  { id: "instagram", label: "Insta", Icon: Image },
  { id: "tiktok", label: "TikTok", Icon: Music2 },
  { id: "youtube", label: "Shorts", Icon: Video },
...
export default function BigWinShareDialog({ open, onClose, detail }: Props) {
  ...
  const handleChannel = useCallback(
    async (ch: ShareChannel) => {
      if (!detail) return;
      const url =
        typeof window !== "undefined"
          ? `${window.location.origin}/?ref=share_${ch}`
          : "https://phonara.world";
      ...
      const { data, error } = await supabase.rpc("claim_share_reward" as any, {
        _channel: ch,
      });
      if (error) {
      ...
      if (d?.ok && !d?.already_claimed)
        notify.success(`+${fmt(d.amount)} PHON 공유 보상`);
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md p-0 bg-card border-primary/30 overflow-hidden">
        ...
        <DialogDescription className="sr-only">
          방문 큰 승리를 8개 채널에 공유하고 +200 PHON씩 받으세요.
        </DialogDescription>
        <AnimatePresence>
          {open && (
            <motion.div
            ...
            >
              <div className="px-5 pt-5 pb-3">
                <div className="text-[10px] tracking-[0.3em] font-black text-primary uppercase">
                  BIG WIN
                </div>
                <div className="font-bold text-foreground text-lg mt-1">
                  방금 +{fmt(detail?.amount ?? 0)} 적중!
                </div>
                <div className="text-xs text-muted-foreground">
                  8개 채널 공유 시 채널당 +200 PHON
                </div>
              </div>
                ...
                onClick={async () => {
                  ...
                  try {
                    await navigator.clipboard.writeText(
                      `${typeof window !== "undefined" ? window.location.origin : ""}/?ref=share`,
                    );
                  } catch {
                    /* */
                  }
