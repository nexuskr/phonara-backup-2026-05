...
import { X, Send, MessageCircle, Copy, Sparkles } from "lucide-react";
...
export default function ForcedShareDialog() {
  ...
  const refCode = useMemo(() => {
    try {
      return localStorage.getItem("pm_my_ref_code") ?? undefined;
    } catch {
      return undefined;
    }
  }, [open]);
  ...
  const text = payload
    ? `${payload.title}${payload.hashtag ? " " + payload.hashtag : ""}`
    : "";
