import { Gem, TrendingUp, Bot, Gift } from "lucide-react";
      ...
      <div className="rounded-2xl border border-gold/30 bg-background/65 backdrop-blur-md p-3 sm:p-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        {ITEMS.map((it) => {
          ...
          return (
            <div
              key={it.title}
              className="flex flex-col items-center text-center gap-1 px-1 py-2"
            >
              ...
              <div
                className={`text-[10px] sm:text-xs font-black tracking-widest ${it.tone}`}
              >
                {it.title}
              </div>
              <div className="text-[11px] sm:text-xs font-bold text-foreground/90 leading-tight">
                {it.line1}
              </div>
