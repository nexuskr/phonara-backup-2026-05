...
const HistoryRow = memo(HistoryRowImpl, (a, b) =>
  a.t.id === b.t.id && a.t.pnl === b.t.pnl && a.unit === b.unit
);

function TradingHistoryGoldImpl({ history, unit = "USDT" }: { history: LiveTrade[]; unit?: Unit }) {
  if (!history.length) {
    return (
      <EmptyState
  ...
  return (
    <div className="space-y-2">
      {history.map((t) => <HistoryRow key={t.id} t={t} unit={unit} />)}
    </div>
  );
}

export default memo(TradingHistoryGoldImpl);
