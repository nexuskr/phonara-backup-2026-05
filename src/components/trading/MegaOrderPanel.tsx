...
function MegaOrderPanelImpl({
  ...
  useEffect(() => {
    setLeverage(loadSymbolLeverage(symbol, 20));
  }, [symbol]);
  useEffect(() => {
    saveSymbolLeverage(symbol, leverage);
  }, [symbol, leverage]);
  ...
  const longEntry = useMemo(
    () => (price ? applySlippage("long", price, true) : 0),
    [price],
  );
  const shortEntry = useMemo(
    () => (price ? applySlippage("short", price, true) : 0),
    [price],
  );
  const sizeLong = useMemo(
    () => computeSize(marginNum, leverage, longEntry),
    [marginNum, leverage, longEntry],
  );
  const sizeShort = useMemo(
    () => computeSize(marginNum, leverage, shortEntry),
    [marginNum, leverage, shortEntry],
  );
  const liqLong = useMemo(
    () => liquidationPrice("long", longEntry, leverage),
    [longEntry, leverage],
  );
  const liqShort = useMemo(
    () => liquidationPrice("short", shortEntry, leverage),
    [shortEntry, leverage],
  );
  const fee = useMemo(
    () => openFee(marginNum, leverage),
    [marginNum, leverage],
  );
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "l" || e.key === "L") {
        e.preventDefault();
        if (!busy) doSubmit("long");
      } else if (e.key === "s" || e.key === "S") {
        e.preventDefault();
        if (!busy) doSubmit("short");
      }
    };
  ...
  return (
    <section
      className={`relative glass-strong rounded-3xl p-4 sm:p-6 space-y-4 border ${
        mode === "real"
          ? "border-amber-400/40 shadow-[0_0_60px_rgba(244,180,55,0.15)]"
          : "border-cyan-400/30"
      }`}
    >
      {hot && mode === "real" && (
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 relative">
        <div className="flex items-center gap-2">
          <Zap
            className={`w-4 h-4 ${mode === "real" ? "text-amber-300" : "text-cyan-300"}`}
          />
          <h2 className="font-display font-black text-lg tracking-wide">
            MEGA ORDER
          </h2>
          <span
          ...
          <div className="inline-flex rounded-lg border border-border/50 bg-background/60 p-0.5 text-[10px] font-black">
            {(["isolated", "cross"] as const).map((m) => (
              <button
                key={m}
                onClick={() => {
                  if (m !== marginMode) setPendingMode(m);
                }}
      ...
      <div>
        <div className="flex items-baseline justify-between">
          <label className="text-xs text-muted-foreground">
            {marginMode === "isolated"
              ? "Allocated Margin (이 포지션 전용)"
              : "Margin"}{" "}
            ({unit === "KRW" ? "원화 / KRW" : "USDT"})
            <span className="ml-2 text-[10px] text-muted-foreground/70">
              잔액 {fmtMoney(balance, unit)}
            </span>
          </label>
        ...
        <div className="mt-1 text-[10px] text-muted-foreground/70">
          ≈{" "}
          {fmtMoney(cross.value, cross.unit, {
            decimals: cross.unit === "USDT" ? 2 : 0,
          })}
        </div>
      </div>
      ...
      <div>
        <div className="flex items-baseline justify-between">
          ...
          <div
            className={`flex items-center gap-1 text-2xl font-black tabular-nums ${hot ? "text-red-400" : "text-amber-300"}`}
          >
      ...
      <div className="rounded-2xl border border-border/50 bg-background/30 p-3 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black tracking-wide uppercase text-muted-foreground">
            TP / SL / Trailing
          </span>
        ...
        {(tpNum > 0 || slNum > 0) && marginNum > 0 && (
          <div className="grid grid-cols-2 gap-2 pt-1 border-t border-border/40">
            <div className="rounded-lg bg-emerald-500/5 border border-emerald-500/20 p-2">
              <div className="text-[10px] text-emerald-300/80 uppercase tracking-wider">
                Est. TP PnL
              </div>
              <div className="font-mono tabular-nums font-black text-emerald-300 text-sm">
                {tpNum > 0
                  ? `+${fmtMoney(estTpPnl, unit, { decimals: unit === "USDT" ? 2 : 0 })}`
                  : "—"}
              </div>
            </div>
            <div className="rounded-lg bg-rose-500/5 border border-rose-500/20 p-2">
              <div className="text-[10px] text-rose-300/80 uppercase tracking-wider">
                Est. SL PnL
              </div>
              <div className="font-mono tabular-nums font-black text-rose-300 text-sm">
                {slNum > 0
                  ? fmtMoney(estSlPnl, unit, {
                      decimals: unit === "USDT" ? 2 : 0,
                    })
                  : "—"}
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
        ...
        <Stat
          label="Size"
          v={`${sizeLong.toFixed(4)}/${sizeShort.toFixed(4)}`}
        />
        <Stat
          label="Fee (0.1%)"
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button
          ...
          className="h-20 text-xl font-display font-black bg-linear-to-b from-emerald-400 to-emerald-600 hover:from-emerald-300 hover:to-emerald-500 text-emerald-50 shadow-[0_0_40px_rgba(52,211,153,0.5)] border border-emerald-300/40 disabled:opacity-60"
        >
        ...
        <Button
          ...
          className="h-20 text-xl font-display font-black bg-linear-to-b from-rose-500 to-rose-700 hover:from-rose-400 hover:to-rose-600 text-rose-50 shadow-[0_0_40px_rgba(244,63,94,0.5)] border border-rose-400/40 disabled:opacity-60"
        >
      ...
      <p className="text-[10px] text-muted-foreground/80 text-center">
        ...
        {stale && (
          <span className="ml-2 text-amber-300">· 시장가 동기화 대기 중</span>
        )}
      </p>
...
function Stat({
  ...
  return (
    <div
      className="rounded-xl bg-background/40 border border-border/40 p-2"
      title={tooltip}
    >
      <div className="text-[10px] text-muted-foreground uppercase tracking-wider flex items-center gap-1">
        ...
        {tooltip && (
          <span
            aria-hidden
            className="inline-flex items-center justify-center w-3 h-3 rounded-full border border-muted-foreground/40 text-[8px] font-bold cursor-help"
          >
            ?
          </span>
        )}
      ...
      <div
        className={`font-mono tabular-nums font-bold mt-0.5 text-sm ${
          tone === "loss"
            ? "text-rose-300"
            : tone === "warn"
              ? "text-amber-300"
      ...
      {sub && (
        <div className="text-[10px] text-muted-foreground/70 font-mono tabular-nums mt-0.5">
          {sub}
        </div>
      )}
    </div>
