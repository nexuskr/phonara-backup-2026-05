...
import {
  useTriggerStore,
  type PositionTriggers,
} from "@/lib/trading/triggers-store";
import { notify } from "@/lib/notify";
import LiquidationReplayModal, {
  type ReplayPayload,
} from "./LiquidationReplayModal";
...
function OpenPositionsLiveImpl({
  ...
  const buildReplay = (
    ...
    pnl: number,
    roi: number,
    reason: ReplayPayload["reason"],
  ): ReplayPayload => {
    const feeOpen =
      Number(p.fee_open ?? 0) || Math.floor(p.margin * p.leverage * FEE_RATE);
  ...
  const total = useMemo(
    () =>
      positions.reduce(
        (s, p) =>
          s + computePnl(p.side, p.entry, prices[p.symbol] ?? p.entry, p.size),
        0,
      ),
    [positions, prices],
  );
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2 px-1">
          <div className="text-xs text-muted-foreground">
            ...
            <span
              className={`font-mono font-bold ${total >= 0 ? "text-emerald-400" : "text-rose-400"}`}
            >
              {total >= 0 ? "+" : ""}
              {fmt(total)}
            </span>
        ...
        {positions.map((p) => (
          <PositionRow
            ...
            onClose={async (id, mark) => {
              ...
              if (!("error" in r)) {
                const reason: ReplayPayload["reason"] =
                  r.roi <= -0.99 ? "liquidation" : "manual";
...
function PositionRowImpl({ position: p, mark, busy, unit, onClose }: RowProps) {
  ...
  const tpPrice = triggers?.tpPct
    ? priceFromRoi(p.side, p.entry, p.leverage, triggers.tpPct)
    : 0;
  const slPrice = triggers?.slPct
    ? priceFromRoi(p.side, p.entry, p.leverage, -triggers.slPct)
    : 0;
  ...
  const [tpInput, setTpInput] = useState<string>(
    triggers?.tpPct ? String(triggers.tpPct) : "",
  );
  const [slInput, setSlInput] = useState<string>(
    triggers?.slPct ? String(triggers.slPct) : "",
  );
  const [trailInput, setTrailInput] = useState<string>(
    triggers?.trailingPct ? String(triggers.trailingPct) : "",
  );
  ...
  useEffect(() => {
    ...
    setTpPriceInput(
      triggers?.tpPct
        ? priceFromRoi(p.side, p.entry, p.leverage, triggers.tpPct).toFixed(4)
        : "",
    );
    setSlPriceInput(
      triggers?.slPct
        ? priceFromRoi(p.side, p.entry, p.leverage, -triggers.slPct).toFixed(4)
        : "",
    );
  ...
  const saveTriggers = () => {
    ...
    if (!tp && !sl && !tr) {
      ...
      setTrigger(p.id, {
        ...
        slPct: sl,
        trailingPct: tr,
        peakRoiPct: triggers?.peakRoiPct,
      });
  ...
  return (
    <div
      className={`glass rounded-2xl border p-3 sm:p-4 space-y-2 transition ${
        nearMiss
          ? "border-red-500/70 shadow-[0_0_30px_rgba(239,68,68,0.4)] animate-pulse"
          : positive
    ...
    >
      <div className="grid grid-cols-2 sm:grid-cols-6 gap-2 items-center">
        <div className="text-sm font-black flex items-center gap-1.5 flex-wrap">
          ...
          {p.margin_mode === "cross" ? (
            ...
            <span className="inline-flex items-center px-1.5 py-0.5 rounded-md border border-primary/40 bg-primary/10 text-primary text-[9px] font-bold uppercase tracking-wider">
              Iso
              {p.allocated_margin
                ? ` · ${Math.round(Number(p.allocated_margin))}u`
                : ""}
            </span>
          )}
        </div>
        <div
          className={`text-xs font-black ${p.side === "long" ? "text-emerald-400" : "text-rose-400"}`}
        >
        ...
        <div
          className={`text-sm font-mono tabular-nums font-bold ${positive ? "text-emerald-400" : "text-rose-400"}`}
        >
          {positive ? "+" : ""}
          {fmt(pnl)}
      ...
      <div className="flex justify-between text-[10px] text-muted-foreground">
        ...
        {nearMiss && (
          <span className="text-red-400 font-black flex items-center gap-1">
            <Heart className="w-3 h-3 animate-pulse" /> NEAR LIQUIDATION
          </span>
        )}
      ...
      {!editing &&
        triggers &&
        (triggers.tpPct || triggers.slPct || triggers.trailingPct) && (
          <div className="flex flex-wrap items-center gap-1.5 text-[10px] pt-1 border-t border-border/40">
            {triggers.tpPct ? (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md border border-emerald-500/40 bg-emerald-500/10 text-emerald-300 font-mono tabular-nums">
                <Target className="w-3 h-3" /> TP +{triggers.tpPct}%{" "}
                <span className="opacity-70">@{tpPrice.toFixed(4)}</span>
              </span>
            ) : null}
            {triggers.slPct ? (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md border border-rose-500/40 bg-rose-500/10 text-rose-300 font-mono tabular-nums">
                <ShieldCheck className="w-3 h-3" /> SL −{triggers.slPct}%{" "}
                <span className="opacity-70">@{slPrice.toFixed(4)}</span>
                {slDistPct !== 0 && (
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md border border-amber-500/40 bg-amber-500/10 text-amber-200 font-mono tabular-nums">
                ...
                {triggers.peakRoiPct ? (
                  <span className="opacity-70">
                    peak {triggers.peakRoiPct.toFixed(1)}%
                  </span>
                ) : null}
              </span>
            ) : null}
        ...
        <div className="pt-2 border-t border-border/40 space-y-2">
          <div className="grid grid-cols-3 gap-2">
            <label className="space-y-1">
              <span className="inline-flex text-[10px] text-emerald-300 font-bold items-center gap-1">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setTpPriceInput(
                    v > 0
                      ? priceFromRoi(p.side, p.entry, p.leverage, v).toFixed(4)
                      : "",
                  );
                }}
            ...
            <label className="space-y-1">
              <span className="inline-flex text-[10px] text-rose-300 font-bold items-center gap-1">
            ...
            <label className="space-y-1">
              <span className="inline-flex text-[10px] text-amber-300 font-bold items-center gap-1">
          ...
          <p className="text-[9px] text-muted-foreground/70 leading-relaxed">
            ROI % 또는 가격 어느 쪽이든 입력하세요. 한 쪽을 입력하면 다른 쪽은
            자동 계산됩니다. 비워두면 해당 트리거는 비활성화됩니다.
          </p>
        </div>
      )}
    </div>
