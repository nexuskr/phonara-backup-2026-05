...
import { Flame, Gem, Wallet as WalletIcon } from "lucide-react";

// ==================== 최소 수정 ====================
const Layout = ({ children }: { children: React.ReactNode }) => <>{children}</>;
const HubTabs = (props: any) => (
  <div className="h-12 bg-white/5 flex items-center px-5">Trading Hub</div>
);
...
const ChartWithHeader = (props: any) => (
  <div className="h-96 bg-white/5 rounded-2xl flex items-center justify-center text-white/60">
    📊 Chart Area
  </div>
);
...
const OpenPositionsLive = (props: any) => (
  <div className="p-6 bg-white/5 rounded-2xl">Open Positions</div>
);
const TradingHistoryGold = (props: any) => (
  <div className="p-6 bg-white/5 rounded-2xl">Trading History</div>
);
  ...
  <div className="flex gap-3 mb-6">
    <button
      ...
      className={`px-6 py-3 rounded-2xl ${
        mode === "paper" ? "bg-fuchsia-600 text-white" : "bg-white/10"
      }`}
    >
      Paper
    </button>
    <button
      ...
      className={`px-6 py-3 rounded-2xl ${
        mode === "real" ? "bg-fuchsia-600 text-white" : "bg-white/10"
      }`}
    >
      Real
    </button>
...
function adaptPaperToLive(positions: any) {
  return positions || [];
}
function adaptPaperHistory(history: any) {
  return history || [];
}
