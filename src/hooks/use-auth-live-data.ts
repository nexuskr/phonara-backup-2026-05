...
const KIND_TEMPLATES: Array<
  {
    kind: LiveFeedItem["kind"];
    render: (n: string, a: number) => string;
    amt: () => number;
  }
> = [
  {
    kind: "nft",
    render: (n) => `${n}님이 Legendary NFT를 민팅했습니다!`,
    amt: () => 1,
  },
  {
    kind: "promo",
    render: (n, a) => `${n}님이 PHON 잭팟 +${a.toLocaleString()}!`,
    amt: () => 500 + Math.floor(Math.random() * 5500),
  },
  {
    kind: "tier",
    render: (n) => `${n}님이 등급에 도달했습니다!`,
    amt: () => 9,
  },
  {
    kind: "withdraw",
    render: (n, a) => `${n}님이 ${a.toLocaleString()} PHON 출금 완료`,
    amt: () => 100 + Math.floor(Math.random() * 9900),
  },
];
...
function syntheticFeedItem(seed = Date.now()): LiveFeedItem {
  const tmpl =
    KIND_TEMPLATES[Math.floor(Math.random() * KIND_TEMPLATES.length)];
  const nick = pickNick();
...
function syntheticTop5(): Top5Item[] {
  ...
  while (rows.length < 5) {
    ...
    rows.push({
      ...
      cc,
      flag: FLAG_BY_CC[cc] ?? "🌐",
      score: 9_900_000 - rows.length * 1_100_000 +
        Math.floor(Math.random() * 80_000),
    });
  }
  return rows;
}
...
export function useAuthLiveData() {
  ...
  const [feed, setFeed] = useState<LiveFeedItem[]>(() =>
    Array.from(
      { length: 12 },
      (_, i) => syntheticFeedItem(Date.now() - i * 5000),
    )
  );
  ...
  useEffect(() => {
    ...
    const fetchKpi = async () => {
      ...
      try {
        const { data } = await supabase.rpc(
          "get_world_domination_stats" as any,
        );
        if (!alive || !data) return;
        const d: any = data;
        setKpi((prev) => ({
          active_users: Number(d.active_users_24h ?? prev.active_users) ||
            prev.active_users,
          gmv_24h: Number(d.gmv_24h ?? prev.gmv_24h) || prev.gmv_24h,
          crown_explosion:
            Number(
              d.crown_explosion_24h ?? d.max_crown_24h ?? prev.crown_explosion,
            ) || prev.crown_explosion,
          active_emperors: Number(d.active_emperors ?? prev.active_emperors) ||
            prev.active_emperors,
          new_today:
            Number(
              d.new_today ?? d.signups_24h ?? d.signed_up_today ??
                prev.new_today,
            ) || prev.new_today,
        }));
      } catch { /* silent */ }
    ...
    fetchKpi();
    const t = setVisibleInterval(fetchKpi, 30_000, {
      meta: { owner: "use-auth-live-data", category: "cosmetic" },
    });
    return () => {
      alive = false;
      t();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (isCategoryPaused("cosmetic")) return;
      try {
        const { data } = await supabase.rpc("get_whale_strikes_24h" as any, {
          _limit: 30,
        });
        ...
        const rows: LiveFeedItem[] = (data as any[]).map((r, i) => {
          ...
          const kind: LiveFeedItem["kind"] =
            (["crown", "baron", "withdraw", "nft", "tier"].includes(kindRaw)
              ? kindRaw
              : "crown") as LiveFeedItem["kind"];
          const text = kind === "crown"
            ? `Whale ${nick}님이 Crown Explosion +${amount.toLocaleString()}!`
            : kind === "withdraw"
            ? `${nick}님이 ${amount.toLocaleString()} PHON 출금 완료`
            : kind === "baron"
            ? `${nick}님이 Baron으로 승급했습니다 👑`
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const { data } = await supabase.rpc(
          "get_weekly_referral_leaderboard" as any,
          { _limit: 5 },
        );
        if (!alive || !Array.isArray(data) || data.length === 0) return;
        const rows: Top5Item[] = (data as any[]).map((r, i) => {
          ...
          return {
            ...
            cc,
            flag: FLAG_BY_CC[cc] ?? "🌐",
            score: Number(r.commission_7d ?? r.invited_7d ?? 0) ||
              (9_000_000 - i * 1_000_000),
          };
        });
        setTop5(rows);
      } catch { /* silent */ }
  ...
  useEffect(() => {
    ...
    const id = setVisibleInterval(
      () => {
        ...
        if (n % 4 === 0) {
          setFeed((prev) => [syntheticFeedItem(), ...prev].slice(0, 16));
        }
  ...
  // legacy crown_events realtime handling removed
