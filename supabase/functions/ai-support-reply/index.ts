...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: thread, error: threadErr } = await admin
      ...
      .select("id, user_id, unread_admin")
      .eq("id", threadId)
      .maybeSingle();
    if (threadErr || !thread || thread.user_id !== user.id)
      return json({ error: "thread_not_found" }, 404);
    ...
    if (piiDetected) {
      ...
      if (lastMsg && lastMsg.message === rawMessage) {
        await admin
          .from("support_messages")
          .update({ message: maskedMessage, pii_masked: true })
          .eq("id", lastMsg.id);
      }
      await admin
        .from("support_threads")
        .update({ last_pii_at: new Date().toISOString() })
        .eq("id", threadId);
    }
    ...
    let kbContext = "";
    try {
      const { data: kb } = await admin.rpc("search_support_kb", {
        _query: maskedMessage,
        _limit: 4,
      });
      if (kb && kb.length > 0) {
        kbContext =
            ...
            function: {
              ...
              parameters: {
                ...
                properties: {
                  reply: {
                    type: "string",
                    description: "사용자에게 보여줄 답변(4-5문장 이내)",
                  },
    ...
    let parsed = {
      reply:
        "죄송합니다. 답변을 준비하지 못했어요. 사람 상담사에게 전달드리겠습니다.",
      needs_human: true,
      category: "other",
    };
    try {
    ...
    let finalReply =
      (replyMasked || "").slice(0, 2000) || "곧 상담사가 답변드리겠습니다.";
    ...
    if (piiDetected) {
      ...
      finalReply =
        `🔒 안전을 위해 입력하신 ${kinds} 정보는 자동 마스킹되었습니다.\n\n` +
        finalReply;
    }
    ...
    await admin
      ...
      .update({
        ...
        priority: parsed.needs_human
          ? priority
          : ((thread as any).priority ?? "normal"),
        status: parsed.needs_human ? "reviewing" : "open",
        ...(assignedTo ? { assigned_to: assignedTo } : {}),
        ...(parsed.needs_human
          ? { unread_admin: ((thread as any).unread_admin ?? 0) + 1 }
          : {}),
      })
      .eq("id", threadId);
